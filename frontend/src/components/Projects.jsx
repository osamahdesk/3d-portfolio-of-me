      ...
      <div className="mx-auto max-w-[1320px] px-6 sm:px-10">
        ...
        <div className="grid grid-cols-12 gap-6 mb-20 items-end">
          <div className="col-span-12 md:col-span-7">
            <div className="label-eyebrow mb-5">{c.projectsHeader.eyebrow}</div>
            <h2 className="font-display text-4xl sm:text-5xl lg:text-[64px] leading-[1.02] tracking-tight">
              {c.projectsHeader.titleA}<br />
              <span className="font-serif-italic text-[color:var(--ink-4)]">{c.projectsHeader.titleB}</span>
            </h2>
          </div>
        ...
        <article data-testid="project-hsk" className="relative bg-[color:var(--ink-9)] text-[color:var(--ink-0)] rounded-sm overflow-hidden">
          <div className="grid grid-cols-12 gap-0">
            <div className="col-span-12 lg:col-span-7 p-8 sm:p-12 lg:p-16 border-b lg:border-b-0 lg:border-s border-white/10">
              <div className="flex items-center justify-between">
                <span className="label-eyebrow" style={{ color: "rgba(255,255,255,0.5)" }}>{c.projectHSK.codename}</span>
                <span className="chip" style={{ color: "var(--accent)", borderColor: "var(--accent)" }}>
                  {c.projectsHeader.shipped}
                </span>
              </div>
              <h3 className="font-display text-4xl sm:text-5xl lg:text-[72px] mt-10 leading-[1] tracking-tight">{c.projectHSK.name}</h3>
              <div className="mt-3 font-serif-italic text-2xl text-[color:var(--accent)]">{c.projectHSK.subName}</div>

              <p className="mt-8 text-[16px] leading-[1.95] max-w-xl opacity-85">{c.projectHSK.lead}</p>

              <div className="mt-10 flex flex-wrap gap-2">
                {c.projectHSK.stack.map((s) => <span key={s} className="chip" style={{ color: "var(--ink-0)" }}>{s}</span>)}
              </div>

              <div className="grid grid-cols-2 gap-x-10 gap-y-2 mt-10">
                {c.projectHSK.stats.map((s, i) => (
              ...
              <div className="mt-10">
                <Link to={`/projects/${c.projectHSK.slug}`} data-testid="hsk-case-link" className="btn-pill btn-accent">
                  {c.projectsHeader.viewCase}
            ...
            <div className="col-span-12 lg:col-span-5 p-8 sm:p-12 lg:p-14 bg-[color:var(--ink-8)]">
              <div className="label-eyebrow mb-6" style={{ color: "rgba(255,255,255,0.5)" }}>{c.projectsHeader.coreFeatures}</div>
              <ul className="space-y-5">
                {c.projectHSK.features.map((f, i) => (
        ...
        <article data-testid="project-ai-bot" className="mt-16 grid grid-cols-12 gap-6 items-start">
          <div className="col-span-12 md:col-span-5">
            <div className="sticky top-32">
              <span className="label-eyebrow">{c.projectAIBot.codename}</span>
              <h3 className="font-display text-4xl sm:text-5xl lg:text-[64px] mt-6 leading-[1.02] tracking-tight">{c.projectAIBot.name}</h3>
              <div className="mt-3 font-serif-italic text-xl text-[color:var(--ink-4)]">{c.projectAIBot.subName}</div>
              <p className="mt-6 text-[15px] leading-[1.9] text-[color:var(--ink-4)] max-w-md">{c.projectAIBot.lead}</p>
              <div className="mt-8 inline-flex items-center gap-3 border border-[color:var(--ink-9)] rounded-full px-4 py-2">
                ...
                <span className="font-mono text-[11px] tracking-[0.18em] uppercase">{c.projectAIBot.status}</span>
              </div>
              <div className="mt-8">
                <Link to={`/projects/${c.projectAIBot.slug}`} data-testid="ai-bot-case-link" className="btn-pill btn-light">
                  {c.projectsHeader.viewCase}
                </Link>
              </div>
            </div>
          ...
          <div className="col-span-12 md:col-span-7 space-y-6">
            {c.projectAIBot.visionItems.map((v, i) => (
