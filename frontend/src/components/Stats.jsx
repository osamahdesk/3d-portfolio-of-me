...
export default function Stats() {
  ...
  useEffect(() => {
    const id = setInterval(() => setTIndex((i) => (i + 1) % c.testimonials.length), 6500);
    return () => clearInterval(id);
  }, [c.testimonials.length]);

  const t = c.testimonials[tIndex] || c.testimonials[0];

  return (
    <section ref={sectionRef} data-testid="stats-section" className="relative bg-[color:var(--ink-9)] text-[color:var(--ink-0)] py-28 sm:py-36">
      <div className="mx-auto max-w-[1320px] px-6 sm:px-10">
        <div className="grid grid-cols-12 gap-6 mb-14 items-end">
          <div className="col-span-12 md:col-span-8">
            <div className="label-eyebrow mb-5" style={{ color: "rgba(255,255,255,0.5)" }}>{c.stats.eyebrow}</div>
            <h2 className="font-display text-4xl sm:text-5xl lg:text-[64px] leading-[1.02] tracking-tight">
              {c.stats.titleA}<br />
              <span className="font-serif-italic text-[color:var(--accent)]">{c.stats.titleB}</span>
            </h2>
          </div>
          <div className="col-span-12 md:col-span-4 md:text-end opacity-70">
            <p className="text-[14px] leading-[1.85] max-w-xs md:ms-auto">{c.stats.lead}</p>
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 border-b border-white/10">
          {c.audienceStats.map((s, i) => <StatItem key={i} n={s.n} l={s.l} start={start} i={i} />)}
        </div>

        <div className="mt-20 grid grid-cols-12 gap-6 items-end">
          <div className="col-span-12 md:col-span-8">
            <div className="label-eyebrow mb-6" style={{ color: "rgba(255,255,255,0.5)" }}>{c.stats.communityEyebrow}</div>
            <blockquote
              key={tIndex}
            ...
            <div className="flex md:justify-end gap-2">
              {c.testimonials.map((_, i) => (
                <button
                  ...
                  className={`h-[6px] rounded-full transition-all duration-500 ${i === tIndex ? "w-10 bg-[color:var(--accent)]" : "w-3 bg-white/25"}`}
                />
              ))}
            </div>
            <div className="mt-4 font-mono text-[11px] tracking-[0.2em] uppercase opacity-60 md:text-end">
              {String(tIndex + 1).padStart(2, "0")} / {String(c.testimonials.length).padStart(2, "0")}
            </div>
          </div>
        </div>
      </div>

      <style>{`@keyframes fadeSoft { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }`}</style>
    </section>
  );
}
